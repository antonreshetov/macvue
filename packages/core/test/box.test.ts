// @vitest-environment jsdom
import { readFile } from 'node:fs/promises'
import { join } from 'node:path'
import { render } from '@testing-library/vue'
import { axe } from 'vitest-axe'
// Namespace import is required: the package's .d.ts is a type-only re-export,
// so named imports from 'vitest-axe/matchers' do not compile.
import * as matchers from 'vitest-axe/matchers'
import { h } from 'vue'
import { MacBox } from '../src'

// vitest-axe 0.1.0 only augments the pre-vitest-1 `Vi` global namespace,
// so the matcher types are re-declared here for vitest 4.
declare module 'vitest' {
  // eslint-disable-next-line unused-imports/no-unused-vars -- merged declarations must repeat vitest's type parameter
  interface Matchers<T = any> extends matchers.AxeMatchers {}
}

expect.extend(matchers)

describe('macBox', () => {
  it('renders its content inside the box body', () => {
    const { container } = render(MacBox, {
      slots: { default: () => 'Content' },
    })
    expect(container.querySelector('.macvue-box-body')?.textContent).toBe(
      'Content',
    )
    expect(container.querySelector('.macvue-box-title')).toBeNull()
  })

  it('renders the title prop above the body', () => {
    const { container } = render(MacBox, {
      props: { title: 'General' },
      slots: { default: () => 'Content' },
    })
    expect(container.querySelector('.macvue-box-title')?.textContent).toBe(
      'General',
    )
  })

  it('lets the #title slot win over the title prop', () => {
    const { container } = render(MacBox, {
      props: { title: 'Prop title' },
      slots: {
        default: () => 'Content',
        title: () => 'Slot title',
      },
    })
    expect(container.querySelector('.macvue-box-title')?.textContent).toBe(
      'Slot title',
    )
  })

  it('has no axe violations', async () => {
    const { container } = render({
      render() {
        return h(MacBox, { title: 'General' }, { default: () => 'Content' })
      },
    })
    expect(await axe(container)).toHaveNoViolations()
  })

  describe('box.css guards', () => {
    let css: string

    beforeAll(async () => {
      css = await readFile(
        join(import.meta.dirname, '../src/components/box/box.css'),
        'utf8',
      )
    })

    it('is static: no hover, transition or focus styles', () => {
      expect(css).not.toContain(':hover')
      expect(css).not.toContain('transition')
      expect(css).not.toContain(':focus')
    })

    it('uses only tokens: no hex colors, no raw px values', () => {
      expect(css).not.toMatch(/#[0-9a-f]{3,8}/i)
      const pxValues = css.match(/[\d.]+px/g) ?? []
      expect(pxValues).toHaveLength(0)
    })
  })
})
